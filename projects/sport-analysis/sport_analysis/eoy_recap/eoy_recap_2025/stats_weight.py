...
The file with the weight data `2025-weight.csv` was exported from my scale
 Renpho ES-CS20M.
...
# TODO make it parametric by period. And fix the docstring.
...
class WeightStats:
    def __init__(self):
        ...
        self.weights_2025_df: pd.DataFrame = None
    ...
    def finalize_stats(self):
        ...
        self.weights_2025_df = df

    def print_stats(self):
        console.print("[bold on green] > Weight[/]")
        for day, w in self.weights_2025_df.iterrows():
    ...
    def _plot_chart(self, ax: Axes):
        ...
        y = self.weights_2025_df["weight"]
        x = np.arange(len(self.weights_2025_df))
        x_labels = ("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep")
        x_labels += ("Oct", "Nov", "Dec")
        rects = ax.bar(
            x,
            height=y,
            width=width,
        ...
        ax.annotate(
            ...
            xy=(
                (len(self.weights_2025_df) / 2) - 2,
                max(self.weights_2025_df["weight"]),
            ),
    ...
    def _plot_text(self, ax: Axes):
        ...
        weight_avg_2025 = self.weights_2025_df.mean().iloc[0]
        text += f"\n2025 mean: {round(weight_avg_2025, 1)} kg (+{round(weight_avg_2025-self.weight_avg_2024, 1)})"
        ax.annotate(
            text=text,
            # Point to annotate: the top center of the chart.
            xy=(
