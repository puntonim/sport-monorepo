...
@click.command(
    ...
    help='Plot an interval 300m run; eg: san plot-interval-300m-run 18923007987 --vs-n 3 --text "6x300m" --title "6x300m a Verdellino" --figure-size 5.0 8.2 -d ~/workspace/sport-monorepo/projects/sport-analysis/output-images/',
)
...
@click.option(
    ...
    type=int,
    default=6,
    help="Number of expected intervals in the given activity; mind that some intervals in the given activity might have been skipped by accident (and thus they are shorted than 300m and automatically discarded).",
)
...
class PlotInterval300mRunApi(BasePlotIntervalRunApi):
    ...
    # "1x300m 2x300m ... 15x300m"
    DEFAULT_TEXT_TO_SEARCH_FOR_PREVIOUS_ACTIVITIES = (
        "x300m ".join((str(x) for x in range(1, 16))) + "x300m"
    )
    ...
    DEFAULT_N_EXPECTED_INTERVALS = range(3, 16)
